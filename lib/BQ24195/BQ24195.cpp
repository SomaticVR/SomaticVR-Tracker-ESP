...
bool PMICClass::begin()
{
    ...
    //check PMIC version
    if (readRegister(PMIC_VERSION_REGISTER) != 0x2b) {
        return 0;
    }
...
*******************************************************************************/
byte PMICClass::readOpControlRegister(void) { //ne manca uno
...
int PMICClass::readRegister(byte address) {
    ...
    if (_wire->requestFrom(static_cast<uint16_t>(PMIC_ADDRESS), static_cast<uint8_t>(1), true) != 1) {
      return -1;
    }
